# INTERNAL: Temporary notes specific to BrowserUp Devs
...
* Copy all the JAR files from e.g. `mitm/build/libs/...`, ... into the BusDriver repo's `libs` folder.
...
The latest version of BrowserMob Proxy is 3.0.0, powered by [LittleProxy](https://github.com/adamfisk/LittleProxy).
...
## Changes in 3.0.0 since 2.1.4

The LegacyProxyServer is completely removed. No more legacy Jetty backend. Interact with BrowserMobProxy using the `BrowserMobProxyServer` interface or REST API. There is no more browsermob-legacy package. The `--use-littleproxy` command line argument is removed; little proxy is always used now.

The following REST API endpoints no longer available:

 - GET /:port/limits
 - PUT /:port/limit no longer accepts these arguments: upstreamMaxKB, downstreamMaxKB, payloadPercentage, maxBitsPerSecond, enable
                                                              }
...
Limit the bandwidth through the proxy on the *[port]* | PUT | */proxy/[port]/limit* |<p>*downstreamKbps* - Sets the downstream bandwidth limit in kbps. Optional.</p><p>*upstreamKbps* - Sets the upstream bandwidth limit kbps. Optional, by default unlimited.</p><p>*latency* - Add the given latency to each HTTP request. Optional, by default all requests are invoked without latency.</p>
...
You'll find the standalone BrowserMob Proxy distributable zip at `browsermob-dist/target/browsermob-proxy-3.0.0-SNAPSHOT-bin.zip`. Unzip the contents and run the `browsermob-proxy` or `browsermob-proxy.bat` files in the `bin` directory.
